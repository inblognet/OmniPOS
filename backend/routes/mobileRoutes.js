...
router.post('/auth/login', async (req, res) => {
    ...
    try {
        ...
        console.log('? User found:', userData.email, 'Type:', userType);
        
        // Verify password
        let isValid = false;
        if (userType === 'customer') {
            // Simple password comparison for customers
            isValid = userData.password === password;
            console.log('Customer password check:', isValid ? '? Valid' : '? Invalid');
        } else {
            // bcrypt comparison for staff
            isValid = await bcrypt.compare(password, userData.password);
            console.log('Staff password check:', isValid ? '? Valid' : '? Invalid');
        }
        ...
        await client.query(
            ...
            [userData.id, userType, sessionToken, device_id || null, device_name || null, device_model || null, os_version || null, req.ip || null]
        );
        ...
        const responseUser = {
            ...
            role: userData.user_type === 'staff' ? userData.user_type : null
        };
...
router.get('/customer/dashboard', async (req, res) => {
    ...
    try {
        ...
        res.json({
            success: true,
            stats: customerStats.rows[0] || { total_spent: 0, total_orders: 0, loyalty_points: 0, loyalty_joined: false },
...
// Test endpoint to verify mobile routes are working
