...
export default function OrderDetailPage() {
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      const res = await api.get(`/mobile/orders/${orderId}`);
      ...
      if (res.data.success) {
        ...
        const orderData = {
          ...res.data.order,
          total_amount: parseFloat(res.data.order.total_amount) || 0,
          discount_amount: parseFloat(res.data.order.discount_amount) || 0,
        };

        const itemsData = res.data.items.map((item: any) => ({
          ...item,
          ...
          <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden">
            {items.length === 0 ? (
              ...
              items.map((item, index) => (
                <div
                  key={item.id}
                  className={`p-4 flex justify-between items-center ${index !== items.length - 1 ? 'border-b border-gray-100' : ''}`}
                >
